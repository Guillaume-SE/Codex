...
export default class MoviesController {
  ...
  public async updateOneMovie({ request, params, response }: HttpContextContract) {
    ...
    const movieToUpdate = await MovieInfo.findBy("mediaId", mediaId)
    const movieDoesntExist = !movieToUpdate
    if (movieDoesntExist) {
      return response.status(404).json("Aucun film ne correspond à ce media")
    }
    ...
    try {
      ...
      movieToUpdate.merge(specificMovieInfos).save()
      ...
      return response
        .status(201)
        .json({ media: mediaToUpdate, movieInfos: movieToUpdate, cover: actualCover })
    } catch (error) {
      await trx.rollback()
      return response.status(400).json(error)
    }
